...
import { Folder, Workspace as workspace, File } from '@prisma/client';
...
const QuillEditor: React.FC<QuillEditorProps> = ({
  ...
  const deleteBanner = async () => {
    ...
    if (dirType === 'file') {
      ...
      if (details.bannerUrl)
        await edgestore.myPublicImages.delete({ url: details.bannerUrl });
  ...
  useEffect(() => {
    ...
    socket.emit('create-room', fileId, user?.id, user?.email);
  }, [socket, fileId]);
  ...
  useEffect(() => {
    ...
    socket.on('update-collaborators', (collaborators: Collaborator[]) => {
  ...
  useEffect(() => {
    ...
    if (localCursors.length > 0) {
      localCursors.forEach((cursorId: string) => {
        cursorsModule.removeCursor(cursorId);
      });
    }
    
    const newCursorIds: string[] = [];
    collaborators.forEach((collaborator: any) => {
      if (collaborator.userid !== user?.id) {
        ...
        newCursorIds.push(collaborator.userid);
      }
    });
    
    setLocalCursors(newCursorIds);
    
    return () => {
      newCursorIds.forEach(cursorId => {
        cursorsModule.removeCursor(cursorId);
      });
    };
  }, [fileId, quill, user, collaborators]);
  ...
  return (
    <>
      <div className="relative">
        ...
        <div
          className="flex flex-col-reverse justify-center p-8 sm:flex-row sm:justify-between sm:items-center sm:p-2"
        >
          <div>{breadCrumbs}</div>
          <div className="flex items-center gap-4">
                ...
                <TooltipProvider key={index}>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Avatar
                        className="flex items-center hover:cursor-pointer justify-center w-8 h-8 -ml-3 border-2 border-white rounded-full bg-background"
                      >
                        <AvatarImage
                          src={
            ...
            {saving ? (
              <Badge
                variant="secondary"
                className="z-50 text-white bg-orange-600 top-4 right-4 "
              >
                Saving...
              </Badge>
            ) : (
              <Badge
                variant="secondary"
                className="z-50 text-white bg-emerald-600 top-4 right-4 "
              >
                Saved
              </Badge>
            )}
      ...
      {details.bannerUrl && (
        <div className="relative w-full h-[200px]">
            ...
            fill
            className="object-cover w-full h-20 md:h-48"
            alt="Banner Image"
          />
        </div>
      )}
        ...
        >
          ...
          <div className="flex ">
            <BannerUpload
              id={fileId}
              dirType={dirType}
              className="p-2 mt-2 text-sm transition-all rounded-md text-muted-foreground hover:text-card-foreground"
            >
            ...
            {details.bannerUrl && (
              <Button
                ...
                variant="ghost"
                className="flex justify-center gap-2 p-2 mt-2 text-sm rounded-md hover:bg-background item-center text-muted-foreground w-36"
              >
                <XCircleIcon size={16} />
          ...
          <span
            className="text-3xl font-bold text-muted-foreground h-9"
          >
            {details.title}
          </span>
